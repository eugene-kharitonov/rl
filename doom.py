...
class DoomAgent(object):
    def __init__(self):
        ...
        self.episode_states_crop = self.episode_states_rgb[:, 95:115, :, 0:1]
        crop = self.episode_states_crop
        self.thr_crop = tf.select(crop < 200, 255*tf.ones_like(crop), tf.zeros_like(crop))
        batch_size = tf.shape(self.episode_states_rgb)[0]

        #first convolution layer
        W_conv1 = tf.Variable(tf.truncated_normal([5, 5, 1, 8], stddev = 0.1), "Wconv1")
        b_conv1 = tf.Variable(tf.zeros([8]), "bconv1")

        h_conv1 = tf.nn.relu(conv2d(crop, W_conv1) + b_conv1)
        h_pool1 = max_pool_2x2(h_conv1)

        #second convolution layer
        W_conv2 = tf.Variable(tf.truncated_normal([5, 5, 8, 8], stddev = 0.1), "Wconv2")
        ...
        self.entropy = - tf.reduce_mean(tf.log(self.per_action_log_prob) * self.per_action_log_prob)
        self.loss = -tf.reduce_sum((self.rewards - self.V) * tf.log(self.per_action_log_prob)) - 0.01 * self.entropy
    ...
    def end_episode(self):
        ...
        _t1, _t2, v, loss, vloss, entropy = self.sess.run([self.train_step, self.v_train_step, self.V, self.loss, self.value_loss, self.entropy], feed_dict={self.episode_states_rgb: observations, self.rewards: rewards, self.actions: actions})
        ...
        print 'loss', loss, 'value loss', vloss, v.shape, entropy
        #print 'o2', o2 
        #print 'b2', b2
    ...
    def act(self, ob):
        ...
        #[crop, pool] = self.sess.run([self.thr_crop, self.h_pool2], feed_dict={self.episode_states_rgb: ob_as_batch})
        #print pool.shape
        #exit(0)
...
def learn(episodes, max_steps_per_episode, visualisation_freq):
    ...
    for episode in xrange(episodes):
        ...
        j = 0
        last_action = 0
        while not done and j < 5 * 35: #3 sec
            if j % 5 == 0:
                action = agent.act(ob)
                last_action = action
